#include <cstdlib>
#include <ctime>
#include <iostream>
#include <stdlib.h>>

using namespace std;

// get the computer move
char getComputerMove()
{

    int move;

    // generating random number between 0 - 2

    srand(time(NULL));

    move = rand() % 3;



    // returning move based on the random number generated

    if (move == 0) {

        return 'p';

    }

    else if (move == 1) {

        return 's';

    }else{

    return 'r';
    }
}


// Function to return the result of the game

int getResults(char playerMove, char computerMove)
{

    // condition for draw

    if (playerMove == computerMove) {

        return 0;

    }



    // condition for win and loss according to game rule

    if (playerMove == 's' && computerMove == 'p') {

        return 1;

    }

    if (playerMove == 's' && computerMove == 'r') {

        return -1;

    }

    if (playerMove == 'p' && computerMove == 'r') {

        return 1;

    }

    if (playerMove == 'p' && computerMove == 's') {

        return -1;

    }

    if (playerMove == 'r' && computerMove == 'p') {

        return -1;

    }

    if (playerMove == 'r' && computerMove == 's') {

        return 1;

    }



    return 0;
}
// function to get player one move
 char getplayermove1()
 {
    int playermove1;
 }
//function to get player two move
 char getplayermove2()
 {
    int playermove2;
 }
// function to return the result of the game
int getResult(char playermove1, char playermove2)
{
    if(playermove1==playermove2)
        {

        return 0;
       }

    //condition for win and loss according to the game rule


    if(playermove1=='s'&& playermove2=='p')
        {

        return 1;
        }
    if(playermove1=='s'&& playermove2=='r')
        {

        return -1;
       }
    if(playermove1=='p'&& playermove2=='r')
        {

        return 1;
        }
    if(playermove1=='p'&& playermove2=='s')
        {

        return -1;
        }
    if(playermove1=='r' && playermove2=='p')
        {

        return -1;
        }
    if(playermove1=='p' && playermove2=='s')
        {

        return -1;
        }
    if(playermove1=='r' && playermove2=='s')
        {
        return 1;
        }

    return 0;
}


// driver code

int main()
{
    char playerMove ,playermove1,playermove2,ComputerMove;
    string choice;
    int num ;

    p:
    cout<<"\t\t1. To play with the computer"<<endl;

    cout<<"\t\t2. To play with two player"<<endl;

    cout<<"\t\t3. To exit the game"<<endl;

    cin>>num;
    switch( num)
    {

     case 1:
         {
             a:

     cout << "\n\n\n\t\t\tWelcome to rock Paper Scissor "

            "Game\n";
            cout << "\n\t\tEnter r for ROCK, p for PAPER, and s "

            "for SCISSOR\n\t\t\t\t\t";


    // input from the user

    while(1) {

        cin >> playerMove;

        if (playerMove == 'p' || playerMove == 'r' || playerMove == 's') {

            break;

        }

        else {

            cout << "\t\t\tInvalid Player Move!!! Please Try next time." << endl;
            exit(0);

        }

    }


    // computer move


    char computerMove = getComputerMove();




    int result = getResults(playerMove, computerMove);




    // priting result based on who won the game


    if (result == 0) {

         cout << "\n\t\t\tGame Draw!\n";

        cout<<"\t\t\tYour Move:"<<playerMove<<endl;

          cout<<"\t\t\tComputer Move;"<<computerMove<<endl;

          cout<<"\n\t\tDo you want to play again? Enter YES or NO"<<endl;

          cin>>choice;
            if(choice=="YES" || choice=="yes")

                goto p;

            else{
                cout<<"\t\tIt was great to have you, See You Next Time"<<endl;
            }

    }

    else if (result == 1) {

        cout << "\n\t\t\tCongratulations! Player won the "

                "game!\n";
         cout<<"\t\t\tYour Move:"<<playerMove<<endl;

          cout<<"\t\t\tComputer Move;"<<computerMove<<endl;

          cout<<"\n\t\tDo you want to play again? Enter YES or NO"<<endl;

          cin>>choice;
            if(choice=="YES" || choice=="yes")

                goto p;

            else{
                cout<<"\t\tIt was great to have you, See You Next Time"<<endl;
            }


    }

    else {

        cout << "\n\t\t\tOh! Computer won the game!\n";


          //showing both move
          cout<<"\t\t\tYour Move:"<<playerMove<<endl;
          cout<<"\t\t\tComputer Move;"<<computerMove<<endl;
          goto p;

         }
        break;

         }

     case 2:
         {
            b:

        cout << "\n\n\n\t\t\tWelcome to rock Paper Scissor TWO players mode "

            "Game\n";

            cout << "\n\t\tEnter r for ROCK, p for PAPER, and s "

            "for SCISSOR\n\t\t\t\t\t";


    // input from the user


    while(1) {

        cout<<endl<<"Enter your move player 1"<<"\t";
        cin>>playermove1;
        cout<<endl<<"Enter your move player 2"<<"\t";
        cin>>playermove2;
        if(playermove1=='p'||playermove1=='r'||playermove1=='s' && playermove2=='r'||playermove2=='s'||playermove2=='p'){
            break;
        }
        else{
            cout<<"\t\t\tInvalid player move!!! please try next time"<<endl;
            exit(0);

    }
    //playermove1




    int result=getResult(playermove1,playermove2);

    //printing result based on who won the game

    if(result==0)
        {
        cout<<"\n\t\t\tGame Draw!\n";
         }
    else if(result==1)
    {
        cout<<"\n\t\t\tCongratulations! player 1 won the game and try next time player 2 ";
         cout<<"\t\t\tPlayer 1 Move:"<<playermove1<<endl;

          cout<<"\t\t\tPlayer 2 Move;"<<playermove2<<endl;

          cout<<"\n\t\tDo you want to play again? Enter YES or NO"<<endl;

          cin>>choice;
            if(choice=="YES" || choice=="yes")

                goto p;

            else{
                cout<<"\t\tIt was great to have you, See You Next Time"<<endl;
            }

    }
    else
    {
        cout<<"\n\t\t\tCongratulations! player 2 won the game and try next time player 1";

         cout<<"\t\t\tPlayer 1 Move:"<<playermove1<<endl;

          cout<<"\t\t\tPlayer 2 Move;"<<playermove2<<endl;

          cout<<"\n\t\tDo you want to play again? Enter YES or NO"<<endl;

          cin>>choice;
            if(choice=="YES" || choice=="yes")

                goto p;

            else{
                cout<<"\t\tIt was great to have you, See You Next Time"<<endl;
            }

    }
          goto b;
         }
        break;

}

       case 3:
           {

           exit(0);
           }
           break;
       default :
        exit(0);
    }

    return 0;
}
